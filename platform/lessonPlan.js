...
async function loadLessonsFromStorage() {
    ...
    try {
        // First try GitHub
        console.log('[DEBUG] Attempting to load lessons from GitHub...');
        const githubLessons = await loadLessonsFromGitHub();
        
        if (githubLessons && githubLessons.length > 0) {
            console.log('[DEBUG] Successfully loaded lessons from GitHub:', githubLessons.length);
            loadedLessons = githubLessons;
        } else {
            console.log('[DEBUG] No lessons found on GitHub, checking localStorage...');
            // Try localStorage as fallback
            const savedLessonsJson = localStorage.getItem('gradeOneLessons');
            if (savedLessonsJson) {
                loadedLessons = JSON.parse(savedLessonsJson);
                console.log('[DEBUG] Loaded lessons from localStorage:', loadedLessons.length);
            }
        }
        ...
        // Update the dropdown
        console.log('[DEBUG] Updating lesson select with lessons:', loadedLessons.length);
        updateLessonSelect();
        
        // Log the current state
        console.log('[DEBUG] Current lessons state:', {
            totalLessons: gradeOneContent.lessons.length,
            lessonNames: gradeOneContent.lessons.map(l => l.name)
        });

    } catch (error) {
        console.error('[ERROR] Failed to load lessons:', error);
        alert('Error loading lessons. Please check the console for details.');
    }
}
...
try {
    // Send the models data to the explore tab
